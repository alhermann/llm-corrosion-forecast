...
reports the aggregated scores.  A key finding is that \textbf{all three
variants perform similarly}, with kNN-only achieving the best overall
IoU (0.9492), slightly ahead of MC LLM+kNN (0.9479) and Deterministic
(0.9453).  The differences are within the standard deviation (0.025--0.028),
indicating that the kNN prior alone captures the dominant dynamics in
latent space for this dataset.
...
\begin{table}[ht]
  ...
  \begin{tabular}{lccc}
    ...
    \midrule
    MC LLM+kNN          & $0.9479 \pm 0.026$ & $0.9731 \pm 0.014$ & $0.676 \pm 0.201$ \\
    Deterministic (T=0)  & $0.9453 \pm 0.028$ & $0.9717 \pm 0.015$ & $0.660 \pm 0.211$ \\
    kNN-only             & $0.9492 \pm 0.025$ & $0.9738 \pm 0.013$ & $0.680 \pm 0.207$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
kNN-only baseline.  The LLM residual provides a marginal positive
contribution at horizons~4--5 (up to $+0.001$) but is slightly
detrimental at shorter horizons.  The deterministic variant shows
larger negative deviations, particularly at horizon~6.

This result suggests that for this dataset and configuration, the kNN
prior already captures the dominant latent dynamics, and the LLM
residual introduces minor perturbations rather than systematic
improvements.  This is an important finding for the thesis discussion:
the LLM's contribution may become more significant with larger state
spaces, more complex corrosion morphologies, or different alloy systems
where purely statistical priors are insufficient.
...
\begin{figure}[ht]
  ...
  The LLM residual provides marginal benefit at mid-range horizons but
  does not consistently outperform kNN-only.}
  \label{fig:delta_iou}
\end{figure}
...
\begin{itemize}[nosep]
  \item \textbf{Brier score} = 0.0262 (lower is better; perfect = 0)
  \item \textbf{NLL} = 0.287 (lower is better)
  \item \textbf{ECE} = 0.027 (lower is better; perfect = 0)
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{kNN prior is a strong baseline}: the inverse-distance weighted
        kNN predictor in normalised PCA space alone achieves competitive
        performance, marginally outperforming the LLM-augmented variants.
  \item \textbf{LLM residual adds marginal value}: the stochastic LLM
        correction provides slight improvements at mid-range horizons but
        does not consistently outperform pure kNN.  This suggests the kNN
        prior already captures the dominant dynamics for this dataset.
  \item \textbf{MC uncertainty is well-calibrated}: Brier = 0.026, ECE = 0.027;
